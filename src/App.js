...
function App() {
  ...
  const decode = (encoded) => {
    ...
    for (let i = 0; i < splitValues.length; i++) {
      if (splitValues[i] < 0) {
        const count = -splitValues[i + 1];
        groupedValues.push(new Array(count).fill(splitValues[i]));
        i++;
      } else {
        groupedValues.push([splitValues[i]]);
      }
    }

    const flattenedValues = groupedValues.flat();

    const result = [];
    let currentIndex = 0;
    for (const value of flattenedValues) {
      if (value < 0) {
        currentIndex += Math.abs(value);
      } else {
        result.push(String.fromCharCode(flattenedValues[currentIndex]));
        currentIndex++;
      }
    }
